        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <template v-for="pasien in store.patients" :key="pasien.id">
                    <tr :class="{ 'bg-blue-50': expandedId === pasien.id }"
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm flex justify-center items-center space-x-2">
                            <button @click="toggleRiwayat(pasien.id)"
                                ...
                                Lihat Riwayat ({{ pasien.medical_records.length }})
                            </button>
...
const formVisibleId = ref(null); // ID pasien yang form-nya sedang terbuka
...
const toggleRiwayat = (id) => {
    // Reset form saat membuka/menutup detail
    resetMedisForm();
    formVisibleId.value = null; // Tutup form saat toggle row
    expandedId.value = expandedId.value === id ? null : id;
};

const toggleAddRecordForm = (id) => {
    if (formVisibleId.value === id) {
        formVisibleId.value = null; // Tutup form
        resetMedisForm();
    } else {
        formVisibleId.value = id; // Buka form
        resetMedisForm();
    }
}
...
const submitMedis = async (patientId) => {
    ...
    try {
        ...
        // Jika sukses, tutup form dan reset state
